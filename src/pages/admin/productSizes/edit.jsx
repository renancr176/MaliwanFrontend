...
export default function AdminEditProductSize() {
  const { id } = useParams();
  const { fireSuccess, fireRequestError, fireConfirm } = useAlert();
  const { getProductSizeById, updateProductSize } = useProductSizeService();
  ...
  const handleSubmit = async (values) => {
    ...
    return updateProductSize(values)
      .then(({ data }) => {
  ...
  return (
    <Container fluid>
      <h3>{t("title")}</h3>
      {data ? (
        <AdminProductSizeForm onSubmit={handleSubmit} data={data} />
      ): (
