...
export default function useAlert() {
  ...
  const mountHtml = (message) => {
    ...
    const isArrayWithMultipleElements =
      Array.isArray(message) && message.length > 1;
  ...
  async function fireConfirm(
    ...
    await Swal.fire({
      ...
      confirmButtonText: confirmButtonText ?? t("confirm.confirmButtonText"),
      cancelButtonText: cancelButtonText ?? t("confirm.cancelButtonText"),
    }).then((result) => {
      conformResult = result.isConfirmed;
    });
